...
class AntFetchPO(env.Env):
  ...
  def reset(self, rng: jp.ndarray) -> env.State:
    ...
    obs_p2 = self._get_obs_p2(qp, info) # p2 OBS
  ...
  @property
  def observation_size_p2(self) -> int:
  ...
  def _get_obs_p2(self, qp: brax.QP, info: brax.Info) -> jp.ndarray:
    """Observation for p2 -- placeholder for now"""
    contact_mag = jp.sum(jp.square(info.contact.vel), axis=-1)
    contacts = jp.where(contact_mag > 0.00001, 1, 0)

    return jp.concatenate([contacts])
