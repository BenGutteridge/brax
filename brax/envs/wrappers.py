...
class EvalWrapper(brax_env.Wrapper):
  ...
  def reset(self, rng: jp.ndarray) -> brax_env.State:
    ...
    eval_metrics = EvalMetrics(
        current_episode_metrics=jax.tree_util.tree_map(jp.zeros_like,
                                             reset_state.metrics),
  ...
  def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:
    ...
    current_episode_metrics = jax.tree_util.tree_multimap(
    ...
    completed_episodes_metrics = jax.tree_util.tree_multimap(
