...
class VelCap(env.Env):
  ...
  def step(self, state: env.State, action: jp.ndarray) -> env.State:
    ...
    p1_force_mult = jp.max(jp.concatenate([jp.zeros(1), action[0] * (1 - (p1_ball_dist_before/max_dist)**2)]))
    p2_force_mult = jp.max(jp.concatenate([jp.zeros(1), action[1] * (1 - (p2_ball_dist_before/max_dist)**2)]))
